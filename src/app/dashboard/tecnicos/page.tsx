    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-1">Gerencie os profissionais de campo e monitore seus cadastros</p>
        </div>
        <button
      ...
      <div className="flex flex-col sm:flex-row gap-4 items-center justify-between bg-slate-900 border border-slate-800 p-4 rounded-2xl">
        <div className="relative w-full sm:max-w-xs">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 rounded-xl bg-slate-950 border border-slate-800 text-white text-sm focus:border-red-500 focus:outline-none transition-colors"
          />
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        {filtered.map((tecnico) => (
          <div
            key={tecnico.id}
            className={`group bg-slate-900 border transition-all duration-300 rounded-2xl p-5 flex flex-col justify-between hover:border-slate-700/80 hover:shadow-xl hover:shadow-slate-950/50 ${
              tecnico.ativo ? 'border-slate-800/80' : 'border-red-950/40 opacity-70'
            }`}
          >
            <div className="space-y-4">
              ...
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-slate-850 to-slate-800 border border-slate-850 flex items-center justify-center font-bold text-red-500 text-base shadow-inner">
                  ...
                  <div>
                    ...
                    <span className="text-xs text-slate-400 font-medium line-clamp-1">{tecnico.cargo}</span>
                  </div>
                </div>
                <button
            ...
            <div className="flex gap-2 mt-5 border-t border-slate-850 pt-3">
              <button
                ...
                className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-xl bg-slate-850 hover:bg-slate-800 text-slate-200 text-xs font-semibold transition-colors border border-slate-800"
              >
                <Edit2 size={13} />
                <span>Editar</span>
              </button>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-slate-900 border border-slate-800 rounded-3xl p-6 w-full max-w-md shadow-2xl shadow-black/80 animate-in fade-in zoom-in-95 duration-200 text-white">
            <h2 className="text-lg font-bold mb-1">
              {isEditing ? 'Editar Técnico' : 'Novo Técnico'}
            </h2>
            <p className="text-slate-400 text-xs mb-5">Preencha os dados do técnico de segurança do trabalho.</p>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-3.5 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-sm focus:border-red-500 focus:outline-none transition-colors"
                />
              </div>

              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1.5">E-mail Profissional</label>
                <input
                  type="email"
                  required
                  value={form.email}
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1.5">Admissão</label>
                  <input
                    type="text"
                    value={form.admissao}
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3.5 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-sm focus:border-red-500 focus:outline-none transition-colors appearance-none"
                >
