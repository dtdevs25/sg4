...
import { getAprs, getAprYears, backfillAprAnoAbertura, upsertAprBatch, deleteAprItem, getAllAprTecnicos } from '@/app/actions/apr'
...
export default function APRPage() {
  ...
  useEffect(() => {
    if (hasLoadedOnce && !showCityDropdown && !showTecnicoDropdown) {
      loadData()
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentPage, itemsPerPage, selectedYear, selectedMonths, selectedRegion, selectedState, selectedCity, selectedTecnico, selectedAtividade, showCityDropdown, showTecnicoDropdown])
  ...
  const handleExportExcel = async () => {
    ...
    try {
      ...
      const resumo = [
        ...
        { label: 'Tempo Médio Abertura', valor: formatDelay(exportStats.avgDelayMs) }
      ]
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center' }}>
                          ...
                          .map(t => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <span style={{ flex: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', fontWeight: isSelected ? 700 : 500, color: isSelected ? '#1e3a8a' : '#334155' }}>{t.nome}</span>
                              </div>
                            )
                          })
                        }
                  ...
                  {selectedTecnico.length > 0 && (
                    <div style={{ padding: '6px 12px', borderRadius: 8, background: '#eff6ff', border: '1px solid #bfdbfe', fontSize: 13, fontWeight: 700, color: '#1e3a8a', display: 'flex', alignItems: 'center', gap: 6 }}>
                      {selectedTecnico.length === 1 ? '1 Técnico selecionado' : `${selectedTecnico.length} Técnicos selecionados`}
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                ...
                <div style={{ background: '#f8fafc', padding: '10px 14px', borderRadius: 10, border: '1px solid #f1f5f9', display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <div>
                    ...
                    <strong style={{ fontSize: 14, fontWeight: 800, color: '#1e293b' }}>{formatDelay(stats.avgDelayMs)}</strong>
                  </div>
                </div>
      ...
      {viewingItem && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(15,23,42,0.8)', padding: 20 }}>
          <div style={{ background: '#fff', borderRadius: 12, width: '100%', maxWidth: 500, boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)' }}>
            ...
            <div style={{ padding: '16px 20px', background: '#660099', borderBottom: '1px solid #4a0072', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderRadius: '12px 12px 0 0' }}>
              ...
              <button onClick={() => setViewingItem(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#fff', opacity: 0.8 }} onMouseEnter={e => e.currentTarget.style.opacity = '1'} onMouseLeave={e => e.currentTarget.style.opacity = '0.8'}>
                <X size={18} />
              </button>
            </div>
