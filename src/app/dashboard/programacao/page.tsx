    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 p-4 rounded-2xl flex flex-col md:flex-row gap-4 items-center justify-between">
        <div className="relative w-full md:max-w-md">
          ...
          <input
            ...
            className="w-full pl-9 pr-4 py-2 rounded-xl bg-slate-950 border border-slate-800 text-white text-xs focus:outline-none focus:border-red-500"
          />
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        {filtered.map((item) => (
          <div
            key={item.id}
            className={`group bg-slate-900 border rounded-2xl p-5 flex flex-col justify-between transition-all hover:border-slate-700 hover:shadow-xl ${
              item.status === 'Concluído' ? 'border-slate-850 opacity-75' : 'border-slate-800'
            }`}
          >
            <div className="space-y-4">
              ...
              {/* Título */}
              <h3 className="font-bold text-white group-hover:text-red-400 transition-colors leading-snug">
                {item.titulo}
              </h3>
              ...
              <div className="space-y-2 border-t border-slate-850 pt-3 text-xs text-slate-400">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-medium text-slate-300">Técnico: {item.tecnico}</span>
                </div>
              </div>
            </div>
            ...
            <div className="flex gap-2 mt-5 border-t border-slate-850 pt-3">
              ...
              <button
                ...
                className="px-3 py-2 rounded-xl bg-slate-850 hover:bg-red-950/30 hover:text-red-400 border border-slate-800 transition-colors"
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-slate-900 border border-slate-800 rounded-3xl p-6 w-full max-w-md shadow-2xl animate-in fade-in zoom-in-95 duration-200 text-white">
            ...
            <p className="text-slate-400 text-xs mb-5">Planeje uma atividade para a equipe técnica.</p>

            <form onSubmit={handleCreate} className="space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1.5">Título do Evento</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1.5">Técnico Encarregado</label>
                <select
                  value={form.tecnico}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-400 uppercase tracking-wider mb-1.5">Local / Planta</label>
                <input
                  type="text"
                  required
